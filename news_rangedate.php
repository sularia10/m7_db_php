...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['not_data'])) {
    ...
    if (!empty($data)) {
        echo "<p>Cerca notícies per data (parcial): <strong>" . htmlspecialchars($data) . "</strong></p>";

        // Consulta segura amb LIKE per buscar coincidències parcials
        $stmt = $db->prepare("SELECT * FROM noticies WHERE not_data LIKE :data");
        $stmt->bindValue(':data', "%$data%", SQLITE3_TEXT); // % per buscar coincidències parcials
        ...
        if (!$trobades) {
            echo "<p style='color:red;'>No s'han trobat notícies amb aquesta data.</p>";
        }
    } else {
        echo "<p style='color:red;'>Si us plau, introdueix una data.</p>";
    }
}
...
<body>
    <form action="" method="POST">
        <label for="not_data">Cerca notícies per data (parcial, per exemple: 2025 o 2025-03):</label>
